...
/* Offset at which super-speed descriptors will start to be added */
#define UVC_SS_DESC_IDX 10
...
struct uvc_data {
	...
	/* Callback for when UVC_VS_COMMIT_CONTROL happens */
	void (*commit_cb)(void *param);
	/* Parameter for this callback */
	void *commit_param;
...
NET_BUF_POOL_VAR_DEFINE(uvc_buf_pool, UVC_TOTAL_BUFS, UVC_TOTAL_BUFS * USBD_MAX_BULK_MPS,
					   ...
					   const uint8_t request)
{
	...
	if (format_desc == NULL || frame_desc == NULL) {
		LOG_ERR("Selected format ID or frame ID not found");
		return -EINVAL;
	}
...
static uint32_t uvc_fmt_pitch(const struct video_format *const fmt)
{
	return video_bits_per_pixel(fmt->pixelformat) * fmt->width / BITS_PER_BYTE;
}

static uint32_t uvc_fmt_size(const struct video_format *const fmt)
{
	return uvc_fmt_pitch(fmt) * fmt->height;
}
				     ...
				     const uint8_t request)
{
	...
	uint32_t max_frame_size = uvc_fmt_size(fmt);
...
static int uvc_set_vs_commit(const struct device *dev, const struct net_buf *const buf)
{
	...
	if (atomic_test_and_set_bit(&data->state, UVC_STATE_STREAM_READY)) {
		atomic_set_bit(&data->state, UVC_STATE_STREAM_RESTART);
	}

	if (data->commit_cb != NULL) {
		data->commit_cb(data->commit_param);
	}
			   ...
			   const bool add_to_fs, const bool add_to_hs, const bool add_to_ss)
{
	...
	if (USBD_SUPPORTS_SUPER_SPEED && add_to_ss) {
...
static int uvc_deassign_all_descs(const struct device *const dev)
{
	...
	/* Clear High Speed descriptor pointers */
	n = data->ss_desc_idx - UVC_SS_DESC_IDX;
	memset(&cfg->ss_desc[UVC_SS_DESC_IDX], 0x00, sizeof(*cfg->ss_desc) * n);
	data->ss_desc_idx = UVC_SS_DESC_IDX;
				     ...
				     const struct video_format *const fmt)
{
	...
	bitrate = (uint64_t)uvc_fmt_size(fmt) * frmival_nsec / (NSEC_PER_SEC / 100);
				 ...
				 const struct video_format *const fmt)
{
	...
	if (format_desc->bDescriptorSubtype == UVC_VS_FORMAT_UNCOMPRESSED) {
		...
		frame_desc->dwMaxVideoFrameBufferSize = uvc_fmt_size(fmt);
		dwFrameInterval = (uint8_t *)&frame_desc->dwFrameInterval;
		bFrameIntervalType = (uint8_t *)&frame_desc->bFrameIntervalType;
...
int uvc_device_enable(const struct device *const dev)
{
	...
	ret = uvc_assign_desc(dev, &cfg->desc->if1_color, true, true, true);
	if (ret != 0) {
		return ret;
	}

	ret = uvc_assign_desc(dev, &cfg->desc->if1_ep_fs, true, false, false);
	if (ret != 0) {
		return ret;
	}

	ret = uvc_assign_desc(dev, &cfg->desc->if1_ep_hs, false, true, false);
	if (ret != 0) {
		return ret;
	}

	ret = uvc_assign_desc(dev, &cfg->desc->if1_ep_ss, false, false, true);
	if (ret != 0) {
		return ret;
	}

	ret = uvc_assign_desc(dev, &cfg->desc->if1_ep_ss_co, false, false, true);
	if (ret != 0) {
		return ret;
	}
